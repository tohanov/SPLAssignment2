...
public class MessageBusTest {
	...
	public void testRegister() {
		assertFalse("Expected to return false before microservice is registered!",
				messageBus.isRegistered(microservice));
	...
	public void testUnregister() {
		...
		assertFalse("should not be subscribed to broadcast after unregister",
				messageBus.isSubscribedToBroadcast(TickBroadcast.class, microservice));
	...
	public void testSubscribeEvent() {
		...
		assertTrue("m1 should be subscribed to trainModelEvent!",
				messageBus.isSubscribedToEvent(TrainModelEvent.class, microservice));
		assertFalse("m1 should not be subscribed to PublishResultsEvent!",
				messageBus.isSubscribedToEvent(PublishResultsEvent.class, microservice));
	}
	...
	public void testSubscribeBroadcast() {
		...
		messageBus.subscribeBroadcast(TickBroadcast.class, microservice);
		assertTrue("m1 should be subscribed to tick broadcast!",
				messageBus.isSubscribedToBroadcast(TickBroadcast.class, microservice));
		assertFalse("m1 should not be subscribed to PublishConferenceBroadcast!",
				messageBus.isSubscribedToBroadcast(PublishConferenceBroadcast.class, microservice));
	}
	...
	public void testSendEvent() {
		...
		assertEquals("message queue should be empty!", 0,
				tesConService.getConference().returnSuccessfulModels().size());
		messageBus.sendEvent(new PublishResultsEvent(
				new Model("testModel", "Images", 1000000, new Student("testStudent", "CS", "PHD"))));
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e1) {
			e1.printStackTrace();
		...
		assertNotEquals("message queue should be empty!", 0,
				tesConService.getConference().returnSuccessfulModels().size());
